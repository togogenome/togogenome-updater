...
tax_check_file = "refseq/current/prokaryotes.tax.json"
...
$output_tsv.puts ["# NCBI taxonomy", "BioProject ID", "RefSeq ID", "Feature", "Locus tag", "Protein ID", "UniProt taxonomy", "UniProt ID", "TogoGenome URI", "UniProt URI"].join("\t")
...
File.open(refseq2up_file).each do |line|
  taxid, bpid, rsid, feature, locus_tag, protein_id = line.strip.split("\t")
  if protein_id
    $pi[protein_id] = line
  else
...
def output(pi_list, taxup, up)
  pi_list.each do |pi|
    taxid, bpid, rsid, feature, locus_tag, = $pi[pi].split("\t")
    if taxid != taxup
...
no_up.each do |pi|
  taxid, bpid, rsid, feature, locus_tag, pi = $pi[pi].strip.split("\t")
  $output_ttl.puts triple("<http://togogenome.org/gene/#{taxid}:#{locus_tag}>", "rdfs:seeAlso", "tax:#{taxid}")
