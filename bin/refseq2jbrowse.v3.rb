...
prefix seqo:  <http://insdc.org/owl/>
...
{
  {
    ...
    where
    {
      ?seq_id seqo:sequence_version ?seq_version filter ( ?seq_version in (%SEQ_VERSIONS%) ) .
      ...
      ?uniqueID rdf:type ?uniqueID_type filter ( ?uniqueID_type in (%SO%) ) .
  ...
  optional {
    ?uniqueID seqo:feature_locus_tag ?name .
  }
  optional {
    ?uniqueID seqo:feature_product ?description .
  }
}
SPARQL
