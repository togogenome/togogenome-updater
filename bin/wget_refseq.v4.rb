    ...
    @refseq_list.each do |entry|
      ...
      Dir.chdir("#{@output_dir}/#{tax_id}/#{prj_id}") {
        ...
        if !(File.exist?("#{seq_id}")) then #ignore already exist
          ...
          wget_command = "wget http://togows.dbcls.jp/entry/nucleotide/#{seq_id}"
          status, stdout, stderr = systemu wget_command unless $DEBUG
          puts stderr 
...
if ARGV.size == 1 
  ...
  puts "./wget_refseq.v4.rb <refseq_list_json> [retry]"
  exit(1)
end
